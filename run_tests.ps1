# ── serez-http Test Runner ───────────────────────────────────────────────────
# Usage: .\run_tests.ps1 [-sz <ruta>] [-filter <str>]
# CWD = serez-http/ → import "src/request" etc. resuelven localmente.
...
Write-Host "serez-http tests ($($tests.Count)) — sz: $sz`n" -ForegroundColor Cyan
$pass = 0; $fail = 0
